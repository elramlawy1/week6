...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void havingEnoughMoney() {
         ...
         */
        // Create a map of item names to their minimum prices
        java.util.Map<String, Integer> itemPriceMap = this.mall.getShopList().stream()
                .flatMap(shop -> shop.getItemList().stream())
                .collect(Collectors.groupingBy(
                        Item::getName,
                        Collectors.collectingAndThen(
                                Collectors.minBy(Comparator.comparing(Item::getPrice)),
                                optionalItem -> optionalItem.get().getPrice()
                        )
                ));
        ...
        List<String> customerNameList = this.mall.getCustomerList().stream()
                .filter(customer -> {
                    int totalCost = customer.getWantToBuy().stream()
                            .mapToInt(item -> itemPriceMap.getOrDefault(item.getName(), 0))
                            .sum();
